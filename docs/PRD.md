...
| 04 | Slice 4: Full victim FSM | ready | dispatch after slice 3 closes; absorbs the new 7-state model |
...
| 08 | Test: VictimFSM | scaffolded | `tests/VictimFSM.test.js` exists with skipped tests; bodies land alongside slice 4 |
...
| 22 | Hybrid map redesign (Happy Hills depth) | **partial: #22a shipped; #22b/c/d/e not started** | #22a — JSON schema (`meta.tile`/`rooms[]`/`links[]`) + multi-leg Victim walker through door tile — shipped. #22b vision cone, #22c hide/crouch/sprint, #22d candle snuffing, #22e crypt room — all not started. |
| 23 | Zoom-in puzzle rooms for clue-hidden evidence | **design-locked; build not started** | New 2026-05-30 PM. 4 sub-slices defined (#23a mouse+camera, #23b puzzle subsystem, #23c evidence reveal wiring, #23d detection + kick-out cutscene). Partially supersedes the "sacristy as separate door-gated room" sub-piece of #22a (#22a's multi-room JSON + walker still load-bearing; #23 changes the *interaction* model for booth + sacristy from walk-through-door to zoom-camera). |
